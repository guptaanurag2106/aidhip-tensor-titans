    ...
    if data_type == "transaction":
        ...
        for i in range(count):
            ...
            result.append({
                ...
                "sentiment": str(round(random.uniform(-1, 1), 2))
            })
        ...
        for i in range(count):
            ...
            result.append({
                ...
                "sentiment_score": str(round(random.uniform(5.0, 9.5), 1)),
...
def main(args):
    ...
    with open(org_file, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "account_balance", "loan_amts", 
            "monthly_spending", "main_expense_categories", 
    ...
    if not org_personas or len(org_personas) < args.org_count:        
        for i in range(len(org_personas) if org_personas else 0, args.org_count):
            ...
            org_personas.append({
                ...
                "account_balance": f"${random.randint(10000, 1000000)}",
                "loan_amts": f"${random.randint(0, 500000)}",
                "monthly_spending": f"${random.randint(5000, 50000)}",
                "main_expense_categories": random.choice(["Technology", "Marketing", "Operations"]),
                "support_interaction_count": str(random.randint(1, 10)),
                "satisfaction": str(random.randint(4, 9)) 
            })
    ...
    for org_id in range(1, args.org_count + 1):
        ...
        for transaction in transactions:
            if not isinstance(transaction, dict):
                continue
            if not transaction.get("expense_category"):
                transaction["expense_category"] = random.choice(["Office Supplies", "Software", "Marketing", "Travel", "Hardware"])
            if not transaction.get("expense_sub_category"):
                transaction["expense_sub_category"] = f"Sub-{transaction.get('expense_category', 'General')}"
        ...
        for post in posts:
            if not isinstance(post, dict):
                continue
            if not post.get("sentiment_score") or post.get("sentiment_score") == "0":
                satisfaction = float(persona.get("satisfaction", 7.0))
                post["sentiment_score"] = str(round(max(0.1, min(10.0, satisfaction + random.uniform(-2.0, 2.0))), 1))
